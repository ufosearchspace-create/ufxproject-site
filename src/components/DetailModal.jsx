...
const DetailModal = ({ sightingId, onClose, onOpenCamera }) => {
  ...
  const loadSightingDetails = async () => {
    ...
    try {
      ...
      // DISABLED: Camera loading temporarily due to API errors
      // Support both lat/lon and latitude/longitude
      // const lat = data.latitude || data.lat;
      // const lon = data.longitude || data.lon || data.lng;
      // if (lat && lon) {
      //   try {
      //     const camerasResult = await getNearbyCameras(lat, lon, 50);
      //     setCameras(camerasResult.data || []);
      //   } catch (err) {
      //     console.warn('Cameras not available:', err.message);
      //   }
      // }
      ...
      <div className="bg-white rounded-lg shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        {loading ? (
          ...
          <div className="p-6 space-y-6">
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <div className="font-semibold">
                    {sighting.datetime || sighting.date || sighting.occurred || 'N/A'}
                  </div>
                </div>
              </div>
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <div className="font-semibold">
                    {sighting.duration_seconds || sighting.duration || 'N/A'} seconds
                  </div>
                </div>
              </div>
            ...
            {(sighting.comments || sighting.description || sighting.summary) && (
              <div>
            ...
            {/* CAMERAS SECTION DISABLED FOR NOW */}
            {/* {cameras.length > 0 && (
              <div>
